...
let btElement;
let wineIdSelected;
let apiURLUsers = "https://cruth.phpnet.org/epfc/caviste/public/index.php/api/users";
...
//Julien : trier vin par année et/ ou pays
document.addEventListener('DOMContentLoaded', function() {
	const urlApi = 'https://cruth.phpnet.org/epfc/caviste/public/index.php/api/wines?key=country&sort=year';
	const selectPays = document.getElementById('listePays');
	const selectAnnee = document.getElementById('listeAnnee');
	const boutonTrier = document.getElementById('sort-button');
	let tousLesVins = [];
	...
	fetch(urlApi)
		...
		.then(donnees => {
			tousLesVins = donnees;
			const ensemblePays = new Set();
			const ensembleAnnees = new Set();

			donnees.forEach(vin => {
				ensemblePays.add(vin.country);
				ensembleAnnees.add(vin.year);
			});


			ensemblePays.forEach(pays => {
				selectPays.innerHTML += `<option value="${pays}">${pays}</option>`;
			});

			ensembleAnnees.forEach(annee => {
	...
	boutonTrier.addEventListener('click', function() {
		...
		afficherVinsFiltres(paysSelectionne, anneeSelectionnee);
	});

	function afficherVinsFiltres(pays, annee) {
		const vinsFiltres = tousLesVins.filter(vin => {
			return (pays === 'Choisissez...' || vin.country === pays) &&
				(annee === 'Choisissez...' || vin.year === annee);
		});

		const listeVins = document.querySelector(".vin");
		listeVins.innerHTML = '';
		vinsFiltres.forEach(vin => {
			const liElementName = document.createElement("button");
			liElementName.innerText = vin.name;
			listeVins.appendChild(liElementName);
...
function Affichage(wine) {
	...
	//Julien  Ajout du bouton Like/Dislike
	let likeButton = document.createElement("button");
	setAttributes(likeButton, {
		id: 'btLike'
	});
	likeButton.innerText = 'Like';
	likeButton.addEventListener('click', () => {
		//console.log(localStorage.getItem('login'), wine.id)
		localStorage.setItem('wineId', wine.id)
		console.log(localStorage.getItem('wineId'))
		console.log(fullAPI)
	});
	ulDescription.appendChild(likeButton);
...
modifyCommentForm.addEventListener('submit', function(event) {
	...
	const wineId = 11; 
	selectedCommentId = btElement; 
	const newCommentContent = newCommentContentInput.value; 
...
//Julien
// Fonction pour gérer le Like/Dislike d'un vin
let fetchLikes = "/likes/wines";
let idUser = '';
let fullAPI = '';
...
fetch(apiURLUsers)
	...
	.then(response => {
		...
		console.log(idUser)
		fullAPI = apiURLUsers + '/' + idUser + fetchLikes;
